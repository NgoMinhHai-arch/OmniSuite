...
async function installDependencies() {
  log('info', 'Tu dong cai dat Node + Python + Playwright (choi quet nguoc: 03_GO_CAI_DAT_OMNISUITE.bat)...');
  const { runFullSetup } = require('./scripts/setup-all-tools');
  ...
  return new Promise((resolve) => {
    ...
    const finish = async () => {
      if (port != null) {
        ...
        if (up) log('ok', `${name} da khoi dong (port ${port})`);
        else log('warn', `${name} chua lang nghe port ${port} — xem log phia tren.`);
      } else {
...
function ensureEnvTokens() {
  ...
  process.env.OMNISUITE_STRICT_SECURITY = process.env.OMNISUITE_STRICT_SECURITY || '1';
  process.env.OMNISUITE_LOCALHOST_ONLY = process.env.OMNISUITE_LOCALHOST_ONLY || '1';
  process.env.OMNISUITE_ANTI_CLONE = process.env.OMNISUITE_ANTI_CLONE || readEnvVar('OMNISUITE_ANTI_CLONE') || '1';
...
async function main() {
  ...
  if (fs.existsSync(path.join(PROJECT_DIR, '.git'))) {
    log('info', 'Git: chay "npm run security:install-hooks" de chan push API key len GitHub.');
  }

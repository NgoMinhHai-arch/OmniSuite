...
def verify_internal_token():
    # Only verify endpoints starting with /api/
    if request.path.startswith('/api/'):
        ...
        # If no token is set in the environment, fallback to the default internal token
        if not expected_token:
            expected_token = "omnisuite_secret_token_123"
        ...
        if not token_passed:
            print(f"[!] SECURITY ALERT: Unauthorized access attempt to Interpreter Service with token: {x_internal_token or authorization}")
            return jsonify({"error": "Unauthorized: Invalid or missing internal token."}), 401

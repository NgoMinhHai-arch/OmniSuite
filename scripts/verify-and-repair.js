...
async function repairPlaywright(log) {
  const { chromiumBrowserReady, runPlaywrightInstall, skipPlaywrightSetup } = require('./setup-all-tools');
  if (skipPlaywrightSetup()) return true;
  if (chromiumBrowserReady()) return true;

  logMsg(log, 'step', 'Cai lai Playwright Chromium...');
  try {
    await runPlaywrightInstall('npx', ['playwright', 'install', 'chromium'], (type, msg) => logMsg(log, type, msg));
    if (!chromiumBrowserReady()) {
      await runPlaywrightInstall('npx', ['playwright', 'install', 'chromium-headless-shell'], (type, msg) =>
        logMsg(log, type, msg),
      );
    }

...
/** Chạy trong trình duyệt (page.evaluate) — tách category từ UI Google Maps mới. */
function extractMapsCategoryInBrowser(): string {
  const clean = (raw: string) => raw.replace(/\s+/g, ' ').trim();

  const fromDotLine = (text: string) => {
    ...
    const first = parts[0];
    const firstIsRating =
      /\d+[.,]\d+/.test(first) || /\(\s*[\d.,\s]+\s*\)/.test(first) || /^\d+[.,]?\d*\s*(star|sao)/i.test(first);
    if (!firstIsRating) {
      if (last.length > 48 || /\d{5,}/.test(last)) return clean(first);
    }
    if (last.length < 2 || last.length > 80) return '';
    if (/^(overview|tổng quan|reviews|đánh giá|about|giới thiệu|prices|giá)$/i.test(last)) return '';
    return clean(last);
  };
  ...
  for (const sel of [
    ...
    'button[aria-label*="Loại"]',
    'button[aria-label*="Danh mục"]',
  ]) {
    const t = document.querySelector(sel)?.textContent?.trim();
    if (t && t.length < 80 && !/^(save|lưu|share|chia)/i.test(t)) return clean(t);
  }

  const header = document.querySelector('[role="main"]') || document.body;
  for (const node of header.querySelectorAll('.fontBodyMedium, .fontBodySmall, .W4Efsd, span')) {
    const t = node.textContent?.trim() || '';
    if (!t || t.length > 120) continue;
    const cat = fromDotLine(t);
    if (cat) return cat;
  }

  const ratingBtn = document.querySelector(
    'button[aria-label*="stars"], button[aria-label*="sao"], button[aria-label*="star"]',
  );
  if (ratingBtn) {
    const row = ratingBtn.closest('.fontBodyMedium')?.parentElement || ratingBtn.parentElement;
    if (row) {
      for (const btn of row.querySelectorAll('button')) {
        if (btn === ratingBtn) continue;
        const t = btn.textContent?.trim() || '';
        const label = btn.getAttribute('aria-label') || '';
        if (t && t.length < 60 && !/star|sao|review|đánh giá/i.test(`${label} ${t}`)) return clean(t);
      }
    }
  }
  ...
  for (const script of document.querySelectorAll('script[type="application/ld+json"]')) {
    try {
      ...
      for (const item of items) {
        ...
        if (typeof type === 'string' && type && !['WebSite', 'WebPage', 'Organization'].includes(type)) {
          return clean(type.replace(/([a-z])([A-Z])/g, '$1 $2'));
        }
        if (Array.isArray(type) && type[0]) return clean(String(type[0]));
      }
    } catch {
      /* ignore */
    }
  ...
  const fromOg = fromDotLine(ogDesc);
...
export async function scrapeGoogleMapsPlaywright({ 
  ...
  try {
    ...
    const categoryByPlaceId = new Map<string, string>();
    ...
    while (collectedHrefs.length < collectTarget) {
      ...
      const feedMeta = await page.$$eval('a[href*="/maps/place/"]', (links) => {
        const fromDotLine = (text: string) => {
          if (!text || (!text.includes('·') && !text.includes('•'))) return '';
          const sep = text.includes('·') ? '·' : '•';
          const parts = text.split(sep).map((p) => p.trim()).filter(Boolean);
          ...
          for (const part of parts) {
            ...
            if (/^\(\s*[\d.,\s]+\s*\)$/.test(part)) continue;
            if (/star|sao/i.test(part) && /\d/.test(part) && part.length < 24) continue;
        ...
        for (const a of links) {
          ...
          const id = href.match(/!1s([^!]+)/)?.[1] || href.split('?')[0];
          if (!id) continue;
          const card = a.closest('[role="article"]') || a.closest('div.Nv2PK');
          let category = '';
          if (card) {
            for (const block of card.querySelectorAll('.W4Efsd, .fontBodyMedium')) {
              category = fromDotLine(block.textContent?.trim() || '');
              if (category) break;
            }
          }
      ...
      for (const { id, category } of feedMeta) {
        if (category && !categoryByPlaceId.has(id)) categoryByPlaceId.set(id, category);
      }
    ...
    for (const href of collectedHrefs) {
      ...
      try {
        ...
        await page.waitForSelector('h1.DUwDvf, h1.fontHeadlineLarge', { timeout: 6000 }).catch(() => {});
        await page
          .waitForFunction(
            () => {
              ...
              return body.includes('·') || body.includes('•') || !!document.querySelector('button.DkEaL, button[jsaction*="category"]');
            },
            { timeout: 5000 },
          )
          .catch(() => {});
        ...
        const placeIdForList = href.match(/!1s([^!]+)/)?.[1] || href.split('?')[0];
        const categoryFromList = categoryByPlaceId.get(placeIdForList) || '';
        ...
        const extractData = async () => {
          ...
          return { name, rating, reviews, address, phone, website, webResults };
        });
          const category = await page.evaluate(extractMapsCategoryInBrowser);
          return { ...details, category: category || '' };
        };

        let data = await extractData();

        // RETRY náº¿u khÃ´ng láº¥y Ä‘Æ°á»£c tÃªn hoáº·c loáº¡i hÃ¬nh
        if (!data.name || (!data.category && !categoryFromList)) {
        ...
        const row: MapsRow = {
          ...
          name: finalName,
          category: data.category || categoryFromList || '',

...
export function buildTaiDownloadAnswer(topic?: string): string {
  ...
  return [
    'TẢI GÓI CHO QUẢN GIA — chỉ khi bạn dùng, không tải sẵn lúc mở app',
    '',
    '1) Clone OmniSuite (đủ rồi)',
    '   git clone https://github.com/NgoMinhHai-arch/OmniSuite.git',
    '   cd OmniSuite && npm install',
    '',
    '2) Lần đầu dùng lệnh — app tự tải:',
    '   /run <việc>           → tải OpenManus',
    '   /run-browser <việc>   → tải browser-use',
    '',
    '3) Tải tay (nếu mạng lỗi):',
    ...
    '',
    '4) Pip runner (tùy chọn, sau khi đã có repo con):',
...
export function buildIntegrationsDownloadPreamble(): string {
  return [
    ...
    '• Clone repo OmniSuite là đủ để bắt đầu.',
    '• OpenManus / browser-use: tải khi /run hoặc /run-browser lần đầu.',
    '• Crawl4AI / Activepieces: lõi cào web & workflow — integrations:fetch khi cần.',
    '• Bảng trạng thái trên máy: /tai-bang',
    '• Hướng dẫn chi tiết: /tai',
    '',
  ].join('\n');
}

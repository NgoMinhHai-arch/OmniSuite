...
export async function fetchModelsForProvider(params: {
  ...
  try {
    ...
    if (provider === 'gemini') {
      ...
      const data = await resp.json();
      if (!resp.ok) {
        const raw = data.error?.message || 'Failed to fetch Gemini models';
        const lower = String(raw).toLowerCase();
        if (lower.includes('api key expired') || lower.includes('expired')) {
          throw new Error(
            'API key Gemini đã hết hạn. Vào Settings → cập nhật gemini_api_key (Google AI Studio).',
          );
        }
        if (lower.includes('api key') || lower.includes('invalid')) {
          throw new Error('API key Gemini không hợp lệ. Kiểm tra lại trong Settings.');
        }
        throw new Error(raw);
      }

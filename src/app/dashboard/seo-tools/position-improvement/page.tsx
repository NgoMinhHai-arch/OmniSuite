...
import { Loader2, Play, Download, ArrowUpRight, ArrowDownRight } from "lucide-react";
...
interface CompareRow { query: string; oldPos: number; newPos: number; delta: number; clicksDelta: number }

export default function PositionImprovementPage() {
  const [siteUrl, setSiteUrl] = useState("");
  const today = new Date();
  const endA = new Date(today); endA.setDate(today.getDate() - 31);
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const mapA = new Map(a.map((r) => [r.keys?.[0] || "", r]));
      const compare: CompareRow[] = b.map((r) => {
        const q = r.keys?.[0] || "";
        const old = mapA.get(q);
        return {
          query: q,
          oldPos: old?.position || 0,
          newPos: r.position || 0,
          delta: (old?.position || 0) - (r.position || 0),
          clicksDelta: (r.clicks || 0) - (old?.clicks || 0),
        };
      }).filter((r) => r.oldPos > 0 && r.newPos > 0).sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta));
      setRows(compare);
    } catch (e) {
  ...
  const handleExport = () => {
    ...
    downloadAsCsv("position-change.csv", ["Query", "Old pos", "New pos", "Delta", "Click delta"],
      rows.map((r) => [r.query, r.oldPos.toFixed(1), r.newPos.toFixed(1), r.delta.toFixed(1), r.clicksDelta]));
  };

  return (
    <ToolShell
      title="Position Improvement"
      description="So sánh vị trí keyword giữa 2 khoảng thời gian (28 ngày trước đó vs 28 ngày gần nhất mặc định)."
      requires={["gsc"]}
    >
      <div className="rounded-2xl p-6 space-y-3" style={{ backgroundColor: "var(--card-bg)", border: "1px solid rgba(6,182,212,0.16)" }}>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            <p className="text-xs font-black uppercase mb-1" style={{ color: "var(--text-muted)" }}>Period A (cũ)</p>
          ...
          <div>
            <p className="text-xs font-black uppercase mb-1" style={{ color: "var(--text-muted)" }}>Period B (mới)</p>
            <div className="flex gap-2">
        ...
        <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--card-bg)", border: "1px solid rgba(6,182,212,0.16)" }}>
          <table className="w-full text-sm">
            <thead className="text-xs font-black uppercase" style={{ color: "var(--text-muted)" }}>
              <tr><th className="text-left p-3">Query</th><th className="text-right p-3">A</th><th className="text-right p-3">B</th><th className="text-right p-3">Δ pos</th><th className="text-right p-3">Δ click</th></tr>
            </thead>
            <tbody>
              {rows.slice(0, 200).map((r) => (
                <tr key={r.query} style={{ borderTop: "1px solid rgba(6,182,212,0.05)" }}>
                  <td className="p-3 truncate max-w-[300px]" style={{ color: "var(--text-primary)" }}>{r.query}</td>
                  <td className="p-3 text-right font-mono">{r.oldPos.toFixed(1)}</td>
                  <td className="p-3 text-right font-mono">{r.newPos.toFixed(1)}</td>
                  <td className={`p-3 text-right font-mono font-bold flex items-center justify-end gap-1 ${r.delta > 0 ? "text-emerald-400" : r.delta < 0 ? "text-rose-400" : ""}`}>
                    {r.delta > 0 ? <ArrowUpRight size={12} /> : r.delta < 0 ? <ArrowDownRight size={12} /> : null}
                    {r.delta > 0 ? "+" : ""}{r.delta.toFixed(1)}
                  </td>
                  <td className={`p-3 text-right font-mono font-bold ${r.clicksDelta > 0 ? "text-emerald-400" : r.clicksDelta < 0 ? "text-rose-400" : ""}`}>

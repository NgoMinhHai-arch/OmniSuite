...
export async function POST(req: Request) {
  try {
    ...
    try {
      logger.info(`[Search API] Calling Python DDGS for: ${selectedKeywords.join(', ')} with mode: ${mode}`);
      ...
      if (pyRes.ok) {
        ...
        if (data.urls && data.urls.length > 0) {
          logger.info(`[Search API] Python backend returned ${data.urls.length} URLs.`);
      ...
      logger.warn(`[Search API] Python backend returned non-OK: ${pyRes.status} - ${errText}`);
    } catch (pyErr: any) {
      logger.warn(`[Search API] Python backend unreachable: ${pyErr.message}`);
    }
  ...
  logger.info('[Search API] Falling back to Bing...');
  ...
  const searchBing = async (keyword: string): Promise<string[]> => {
    ...
    try {
      ...
      logger.error(`[Search API] Bing error for "${keyword}": ${bingErr instanceof Error ? bingErr.message : String(bingErr)}`);
    }
    return found;
  };
  ...
  return NextResponse.json({ error: 'Không tìm thấy kết quả phù hợp. Hãy thử từ khóa khác.' }, { status: 404 });
    ...
    logger.error(`[Search API] Fatal error: ${error instanceof Error ? error.message : String(error)}`);

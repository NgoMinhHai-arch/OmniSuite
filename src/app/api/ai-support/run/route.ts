...
function maxTaskLen(runner: RunnerId): number {
  return RUNNER_MAX_TASK_LEN[runner] ?? 4000;
}
...
function sanitizePayload(body: RunBody, runner: RunnerId): RunBody | null {
  ...
  if (runner === 'open_manus') {
    ...
    if (!task || task.length > maxTaskLen(runner)) return null;
    cleaned.task = task;
  ...
  if (runner === 'job_scraper') {
    const limit = maxTaskLen(runner);
    const jd = (body.jd || body.task || '').toString().trim();
    if (!jd || jd.length > limit) return null;
    cleaned.jd = jd;
    if (body.resume_text) cleaned.resume_text = String(body.resume_text).slice(0, limit * 2);
    return cleaned;
  }
